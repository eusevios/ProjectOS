...
public class AnimeParser extends Parser {
    ...
    void parse() throws IOException, InterruptedException, ParseException {
        ...
        Document calendarPage = Jsoup.connect("https://shikimori.one/ongoings").get();
        List<String> ongoingAnimeURLs = calendarPage.getElementsByClass("cc-entries").select("a").eachAttr("href");
        Document animePage;
        for (String url : ongoingAnimeURLs) {
            ...
            try {
                anime.setGenres(animePage.select("div.key:containsOwn(Жанр)").first().nextElementSibling().select("span.genre-en").eachText().toArray(new String[0]));
            } catch (Selector.SelectorParseException | NullPointerException ignored) {
            }

            try {
                anime.setTopics(animePage.select("div.key:containsOwn(Тем)").first().nextElementSibling().select("span.genre-en").eachText().toArray(new String[0]));
            } catch (Selector.SelectorParseException | NullPointerException ignored) {
            }
            ...
            try {
                String date = animePage.select("div.key:containsOwn(Следующий эпизод:)").first().nextElementSibling().getElementsByClass("local-time").attr("data-datetime");
